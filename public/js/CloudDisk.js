...
CloudDisk.Start = function () {
    ...
    CloudDisk.All.onkeydown = function (e) {
        ...
        if(evt.keyCode===112 ){
            ...
            CloudDisk.SearchBtn.click();
        }else if(evt.keyCode===116){//重新加载、刷新
            CloudDisk.CloudDiskMainInfo( CloudDisk.NowID,CloudDisk.NowType)
        }else if((evt.ctrlKey)&&(evt.keyCode===84)){//打开一个新标签页
            US_Browser.CreatBrower(null);
        }else if((evt.ctrlKey)&&(evt.keyCode===78)){//新建文件夹
            CloudDisk.MouseMenu(CloudDisk.NowID);
            CloudDisk.MouseMenu.CreateFolder()
        }
    };//处理全局键盘事件
    //以上是移入的
        ...
        (function (i) {
            CloudDisk.FuncButton[i].onclick = function () {
                ...
                if(i===0){
                    ...
                    CloudDisk.AddresButton[1].innerHTML = '传输列表';
                    CloudDisk.CloudDiskNavBar.innerHTML = '';
                    CloudDisk.SearchBtn.className = 'sf-icon-search CloudDiskDisable';
                }
            }
    ...
    CloudDisk.All.focus();
    CloudDisk.ControlButton=$(".CloudDiskHeaderControl button");
    CloudDisk.ControlButton[0].onclick=function () {

    };
    CloudDisk.ControlButton[1].onclick=function () {
        ipc.send('disk-mini');
    };
    CloudDisk.ControlButton[2].onclick=function () {
        ipc.send('disk-change');
    };
    CloudDisk.ControlButton[3].onclick=function () {
        ipc.send('disk-close');
    };
    ipcRenderer.on('size',function (event,message) {
        if(message>0){
            CloudDisk.ControlButton[2].className='sf-icon-window-restore';
            CloudDisk.ControlButton[2].setAttribute('tooltip','还原');
        }else {
            CloudDisk.ControlButton[2].className='sf-icon-window-maximize';
            CloudDisk.ControlButton[2].setAttribute('tooltip','最大化');
    ...
    CloudMain.Ajax({
        ...
        success:function (rs) {
            rs=rs[0];
            $(".CloudDiskUser img")[0].src=CloudMain.ServerUrl+'/'+rs.userhead;
            $(".CloudDiskUser span")[0].innerHTML=rs.username;
        }
    })
};//网盘加载入口
CloudDisk.about=function(){
...
CloudDisk.MouseMenuFile = function (thisFolder) {
        ...
        CloudDisk.CreatShare = function () {
            CloudMain.Ajax({
                ...
                success: function (rs) {
                    ...
                    var content = '分享链接已生成，复制给好友吧<br>链接：<input spellcheck="false"  type="text" style="width: 273px" value='+CloudMain.ServerUrl + '/s/' + rs[0].addres + '><br>' + pass + copy;
                    $.Confirm({
                        id: 'CloudDiskConfrim',
                        node: CloudDisk.All,
                        title: '分享链接',
    ...
    CloudDisk.MouseMenuFile.Info = function () {
        CloudDisk.MouseMenuFile.LoadInfo = function (node) {
            ...
            $.Request.load(b.getElementsByTagName("div")[0], CloudMain.ServerUrl+'/service/disk/AddresFile/' + rs.disk_id);
        };
        $.Confirm({
